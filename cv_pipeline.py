...
# --- Tune these under your actual arena lighting using hsv_calibrate.py ---
HSV_LOWER = np.array([32, 119, 126])
HSV_UPPER = np.array([46, 212, 201])

MIN_BALL_AREA = 300       # pixels² — ignore smaller blobs
MIN_CIRCULARITY = 0.65    # 1.0 = perfect circle; lower = more permissive
CONFIRM_FRAMES = 3        # consecutive frames needed to confirm a goal


@dataclass
class BallState:
    detected: bool
    centroid: Optional[Tuple[int, int]]   # (x, y) in pixel space
    radius: float
...
class CVPipeline:
    def __init__(self, goal_left: dict, goal_right: dict, field_polygon: np.ndarray):
        ...
        # Homography (set after calibration; None = use raw pixel coords)
    ...
    def process(self, frame: np.ndarray) -> Tuple[BallState, bool, bool, np.ndarray]:
        """
        Process one frame.
        Returns (ball_state, goal_left_scored, goal_right_scored, debug_frame)
        """
        debug = frame.copy()
        ...
        self._draw_overlays(debug, ball)
        return ball, goal_left, goal_right, debug
    ...
    def _detect_ball(self, frame: np.ndarray) -> BallState:
